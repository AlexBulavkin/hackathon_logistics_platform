...
# Тест с перекрывающимися временными окнами
@patch('app.services.optimization.requests.get', side_effect=dynamic_mock_osrm)
def test_calculate_route_overlapping_time_windows(mock_get):
    overlapping_time_windows_request = {
        ...
        "deliveries": [
            {
                ...
                "refused": False,
                "origin_warehouse": "W1",
                "time_window": [480, 600],  # 8:00 - 10:00
                "service_time": 10
            },
            {
                "id": "D2",
                ...
                "refused": False,
                "origin_warehouse": "W2",
                "time_window": [570, 660],  # 9:30 - 11:00
                "service_time": 15
            }
        ],
        "warehouses": [
    ...
    response = client.post(
        ...
        json=overlapping_time_windows_request
    )
    ...
    # Ожидаем, что обе доставки будут доставлены
    assert len(data["route_order"]) == 2
    assert set(data["route_order"]) == {"D1", "D2"}
